...
export const checkPrimaryKeys = (model: ModelConfig) => {
   ...
   */
  if (extract_primarykeys.length > 1) throw ERROR_MESSAGE.MULTIPLE_SIMPLE_PRIMARY_KEYS;
  ...
  if (extract_primarykeys.length === 1) {
    ...
    if (!VALID_PRIMARY_KEY.includes(primarykey_attr.type)) {
      throw new Error(
        `Type '${primarykey_attr.type}' of primary key '${primarykey_attr.name}' is not valid. The type must be one of ${VALID_PRIMARY_KEY.join(', ')}`,
      );
    }
  }
