...
import {
  checkDependencyInModel as checkModelDependencyModel,
  checkRelationReferences,
} from '../model/checkDependencyInModel';
...
export const deleteElementConfig = async (context: RenderContext<DeleteConfig>, force: boolean) => {
  ...
  if (context.resourceConfig.type === 'controller') {
    ...
    if (context.baseConfig.projectStructureStyle === PROJECT_STRUCTURE_STYLE.DOMAIN_DRIVEN_DESIGN) {
      controllerPath = path.join(
        getDDDControllerDir(context),
        context.resourceConfig.name + 'Controller' + EXTENSIONS.JAVA,
      );
    ...
    const controllerConfigPath = getControllerConfigPath(
      context.resourceConfig.module ?? DIRECTORIES.SHARED,
      context.resourceConfig.name,
      context.basePath,
    );
    ...
    if (await fs.pathExists(controllerConfigPath))
      await fs.rm(controllerConfigPath, { recursive: true });
  ...
  if (context.resourceConfig.type === 'response') {
    ...
    const configPath = getResponseConfigPath(
      context.resourceConfig.module ?? DIRECTORIES.SHARED,
      context.resourceConfig.name,
      context.basePath,
    );
...
const getFilePath = (context: RenderContext<DeleteConfig>) => {
  switch (context.resourceConfig.type) {
    case 'dto':
      if (context.baseConfig.projectStructureStyle === PROJECT_STRUCTURE_STYLE.DOMAIN_DRIVEN_DESIGN)
        return path.join(
          getDDDDtoOutputDir(context),
          `${context.resourceConfig.name}DTO${EXTENSIONS.JAVA}`,
        );
      else
        return path.join(
          getDtoOutputDir(context),
          `${context.resourceConfig.name}DTO${EXTENSIONS.JAVA}`,
        );
    case 'response':
      if (context.baseConfig.projectStructureStyle === PROJECT_STRUCTURE_STYLE.DOMAIN_DRIVEN_DESIGN)
    ...
    case 'enum':
      if (context.baseConfig.projectStructureStyle === PROJECT_STRUCTURE_STYLE.DOMAIN_DRIVEN_DESIGN)
        return path.join(
          getDDDEnumOutputDir(context),
          `${context.resourceConfig.name}${EXTENSIONS.JAVA}`,
        );
      else
        return path.join(
          getEnumOutputDir(context),
          `${context.resourceConfig.name}${EXTENSIONS.JAVA}`,
        );
    default:
      if (context.baseConfig.projectStructureStyle === PROJECT_STRUCTURE_STYLE.DOMAIN_DRIVEN_DESIGN)
