 ...
 * @param context
 */
export const generateCrudServiceInmpl = async (context: RenderContext<CrudControllerConfig>) => {
...
export const renderServiceImpl = async (context: RenderContext<CrudControllerConfig>) => {
  if (context.baseConfig.projectStructureStyle === PROJECT_STRUCTURE_STYLE.DOMAIN_DRIVEN_DESIGN)
    return await renderTemplate(TEMPLATES.DDD_CMD_SERVICE_IMPL, context);
  else return await renderTemplate(TEMPLATES.DOMAIN_CRUD_SERVICE, context);
};

const getServiceImplPath = (context: RenderContext<CrudControllerConfig>) => {
  if (context.baseConfig.projectStructureStyle === PROJECT_STRUCTURE_STYLE.DOMAIN_DRIVEN_DESIGN) {
