...
import { RenderContext, DeleteConfig, ConfigTypes } from "../../interfaces/types";
import { CONFIG_TYPES, DIRECTORIES, configTypeMapping, PROJECT_STRUCTURE_STYLE } from "../../utils/constants";
import { getMainPath, checkIfModuleExist, loadConfig } from "../../utils/helpers";
...
export const checkDependencyInModule = async function (context: RenderContext<DeleteConfig>) {
    ...
    for (const type of CONFIG_TYPES) {
        ...
        const pathConfigType = path.join(basePath, DIRECTORIES.IGRPSTUDIO, moduleName, configTypeMapping[type]);
    ...
    try {
        ...
        if (context.baseConfig.projectStructureStyle === PROJECT_STRUCTURE_STYLE.DOMAIN_DRIVEN_DESIGN) {
            const modulePathSrc = path.join(basePath, getMainPath(context.baseConfig.group, context.baseConfig.packageName), moduleName);
