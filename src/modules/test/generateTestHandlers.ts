...
import {
  ...
  getDDDTestEventHandlerOutputDir,
  getDDDTestQueryHandlerOutputDir,
} from '../../utils/helpers';
...
const getTestHandlerPath = (context: RenderContext<DTOConfig>) => {
  switch (context.resourceConfig.type) {
    case 'command': {
      ...
      return path.join(outputDir, `${context.resourceConfig.name}${COMMAND_HANDLER_SUFFIX}`);
    }
    case 'event': {
      const outputDir = getDDDTestEventHandlerOutputDir(context);
      context.fullPath = outputDir;
      return path.join(outputDir, `${context.resourceConfig.name}${EVENT_HANDLER_SUFFIX}`);
    }
    case 'query': {
