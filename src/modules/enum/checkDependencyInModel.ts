...
export const checkDependencyInModel = async function(context: RenderContext<EnumConfig> | RenderContext<DeleteConfig>) {
    ...
    t.attributes.map(attr => {
      if (attr.objectType === 'enum') {
        ...
        if (type.name === cfg.name) {
          errors.push({message: `'enum.${cfg.name}' is being used in 'model.${t.name}' on attribute line '${attr.name}'.`});
        }
