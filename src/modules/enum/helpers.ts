...
export const verifyEnumAttributes = async function (config: EnumConfig): Promise<EnumConfig> {
  ...
  if (config.values) {
    ...
    const allAttributesEmpty = config.values.every(
      (value) => !value.attributes || value.attributes.length === 0,
    );
    ...
    if (allAttributesEmpty) {
      ...
      const allHaveOneAttributeOrEmpty = config.values.every(
        (value) =>
      ...
      if (allHaveOneAttributeOrEmpty) {
        ...
        config.attributes = config.attributes?.filter(
          (attribute) => toLowerCase(attribute.name) !== 'description',
        );
      }
    }
  }
