...
export const checkDependencyInController = async function (
  context: RenderContext<DTOBaseConfig> | RenderContext<DeleteConfig>,
) {
    ...
    for (const action of controller.actions) {
      if (action.responses)
        for (const response of Object.values(action.responses)) {
          if (
            response.name === dtoName ||
            response.content['application/json']?.schema?.type == dtoName ||
            response.content['multipart/form-data']?.schema?.type == dtoName
          ) {
            errors.push({
