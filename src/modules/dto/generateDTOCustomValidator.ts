...
import { EXTENSIONS, PROJECT_STRUCTURE_STYLE, TEMPLATES } from '../../utils/constants';
import { renderTemplate } from '../common/renderTemplate';
import { saveToFile } from '../common/saveToFile';
import { getDToValidatorDir, getDToValidatorDirDDD } from '../../utils/helpers';
import { normalizeImplValidatorName, normalizeInterfaceValidatorName } from './helpers';
...
const getValidatorInterfacePath = (context: RenderContext<DTOBaseConfig>) => {
  ...
  const interfaceValidatorName =
    normalizeInterfaceValidatorName(context.resourceConfig.name) + EXTENSIONS.JAVA;
...
const getValidatorImplPath = (context: RenderContext<DTOBaseConfig>) => {
  const outputDir = getValidatorDirectory(context);
  context.fullPath = outputDir;
  const implValidatorName =
    normalizeImplValidatorName(context.resourceConfig.name) + EXTENSIONS.JAVA;
